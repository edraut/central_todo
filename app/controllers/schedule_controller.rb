class ScheduleController < ApplicationController
  ...
  def index
    ...
    sql = %Q`SELECT projects.id as id, "projects".title as title, projects.state as state, projects.user_id as user_id, projects.due_date as due_date, 'Project' as item_type FROM "projects" WHERE ("projects".user_id = #{@this_user.id}) AND (projects.state = 'active') AND (projects.due_date is not null) UNION SELECT tasks.id as id, "tasks".title as title, tasks.state as state, tasks.user_id as user_id, tasks.due_date as due_date, 'Task' as item_type FROM "tasks" WHERE ("tasks".user_id = #{@this_user.id}) AND (tasks.state = 'active') AND (tasks.due_date is not null) ORDER BY due_date LIMIT 25 OFFSET #{offset};`
